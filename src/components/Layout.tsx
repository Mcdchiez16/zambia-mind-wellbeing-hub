...
const Layout = () => {
  ...
  const headerBg = isHomePage && !scrolled ? "bg-transparent" : "bg-background/95 backdrop-blur-md border-b border-border shadow-sm";
  return <div className="min-h-screen flex flex-col bg-background">
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${headerBg}`}>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex items-center gap-1">
              <span className="text-2xl font-bold tracking-tight">
                ...
                <span className="text-accent font-mono"> Mind</span>
              </span>
            </Link>

            {/* Desktop Navigation */}
            <nav className="hidden md:flex items-center gap-1">
              {navigationItems.map(item => <Link key={item.path} to={item.path} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${isActive(item.path) ? "bg-primary text-primary-foreground" : isHomePage && !scrolled ? "text-foreground/80 hover:text-foreground hover:bg-foreground/10" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}>
                  {item.icon}
                  {item.name}
                </Link>)}
              ...
              <button onClick={() => setShowHotline(!showHotline)} className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors">
            ...
            <div className="flex items-center gap-2 md:hidden">
              <button onClick={() => setShowHotline(!showHotline)} className="p-2 rounded-full bg-destructive text-destructive-foreground">
                <Phone className="h-4 w-4" />
              </button>
              <ThemeToggle />
              <button className="p-2 rounded-full hover:bg-muted transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu">
        ...
        {/* Mobile Navigation */}
        {isMobile && isMenuOpen && <div className="absolute top-full left-0 right-0 bg-background border-b border-border shadow-lg">
            <nav className="container mx-auto px-4 py-4 flex flex-col gap-2">
              {navigationItems.map(item => <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isActive(item.path) ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-muted"}`}>
                  {item.icon}
